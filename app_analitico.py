...
if pagina == "Painel Analítico":
    ...
    historico_familiar = st.sidebar.multiselect("Histórico Familiar", df["family_history"].unique(), default=df["family_history"].unique())
    lanches = st.sidebar.multiselect("Lanches fora de hora", df["CAEC"].unique(), default=df["CAEC"].unique())
    comida_calorica = st.sidebar.multiselect("Comida Calórica Frequente", df["FAVC"].unique(), default=df["FAVC"].unique())
    ...
    st.subheader("Obesidade x Histórico Familiar")
    ...
    st.subheader("Média de Atividade Física por Categoria de Obesidade")
